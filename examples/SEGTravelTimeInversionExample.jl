...
newSize = [256, 256, 128];
offset  = ceil(Int64,(newSize[1]*(6/13.5)));;
(m,Minv,mref,boundsHigh,boundsLow) = readModelAndGenerateMeshMref(modelDir,"SEGmodel2Dsalt.dat",dim,pad,[0.0,13.5,0.0,13.5,0.0,4.2],newSize,1.752,2.9);
dataFilenamePrefix = string(dataDir,"/shortDATA_SEG",tuple((Minv.n .+ 1)...));
...
if plotting
	figure()
	plotModel(m,includeMeshInfo=true,M_regular = Minv,cutPad=pad,limits=limits,figTitle="mref",filename="orig.png");
	# plotModel(m,true,Minv,pad,limits);
	figure()
	plotModel(mref,includeMeshInfo=true,M_regular = Minv,cutPad=pad,limits=limits,figTitle="mref",filename="mref.png");
...
function dump(mc,Dc,iter,pInv,pMis)
	if resultsFilename!=""
		...
		if plotting
			...
			plotModel(fullMc,includeMeshInfo=true,M_regular = Minv,cutPad=pad,limits=limits,filename=filename,figTitle=filename);
		end
	end
end	
...
D0 = Array{Array{Float64,2}}(undef,length(pMisRFs))
...
Dpred = Array{Array{Float64,2}}(undef,length(pMisRFs))
for k = 1:length(pMisRFs)
	Dpred[k] = fetch(Dc[k]);
end
