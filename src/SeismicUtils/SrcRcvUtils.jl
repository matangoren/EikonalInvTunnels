...
n_linidx = LinearIndices(tuple(n_nodes...) .+ 1);
for k=1:nSrcRcv
	J[k] = k;
	if length(n_nodes)==2
		# I[k] = sub2ind(tuple(n_nodes...),SrcRcvNodeLoc[k,2],SrcRcvNodeLoc[k,3]);
		I[k] = n_linidx[SrcRcvNodeLoc[k,2],SrcRcvNodeLoc[k,3]];
	else
		# I[k] = sub2ind(tuple(n_nodes...),SrcRcvNodeLoc[k,2],SrcRcvNodeLoc[k,3],SrcRcvNodeLoc[k,4]);
		I[k] = n_linidx[SrcRcvNodeLoc[k,2],SrcRcvNodeLoc[k,3],SrcRcvNodeLoc[k,4]];
	end
	V[k] = 1.0;
end
...
function writeSrcRcvLocFile(filename,Msh::RegularMesh,pad::Int64,jump::Int64; type="Src")
...
if Msh.dim==2
	if type == "Src"
		...
		Q[1,pad+1:jump:end-pad] .= 1.0
	end
else
	if type == "Src"
		Q[pad+1:jump:end-pad,pad+1:jump:end-pad,1] .= 1.0;
	else # Rcv
		Q[1,pad+1:jump:end-pad,pad+1:jump:end-pad] .= 1.0;
	end
end
numSrcRcv = round(Int,sum(Q));
Q = findall(Q .!= 0); #find(Q);
...
for k=1:numSrcRcv
	...
	SrcRcvTable[k,2:end] = Tuple(Q[k]).*Msh.h + startLocations;
end
